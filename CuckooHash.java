...
import java.security.DrbgParameters.Capability;
 ...
 public class CuckooHash<K, V> {
     ...
     public void put(K key, V value) {
         ...
         Bucket<K, V> kicked = null; // kicked out node
         Bucket<K, V> current = new Bucket<K, V>(key, value);
         int index = hash1(current.getBucKey());

        
         kicked = table[index]; // kicked gets stored outside the hashmap temporarily
         table[index] = current; // insert new bucket into its spot

         for (int i = 0; i <= CAPACITY; i++) {
             // if kicked is null, nothing needs to be replaced so method terminates
             if (kicked == null){
                return;
             }

             // we kicked a bucket out. was it being housed at its hash1 or hash2 index?
             // if it was at hash1, move it to hash2. if it was at hash2, move it to hash1.
             // and temporarily store whatever is held at that position
             if (hash1(kicked.getBucKey()) == index) { // kicked was stored at its hash1 location, so move it to hash2
                current = kicked;
                index = hash2(current.getBucKey());
                kicked = table[index];
                table[index] = current;
             } else {  //otherwise, kicked was stored at its hash2 location, so move it to hash1
                current = kicked;
                index = hash1(current.getBucKey());
                kicked = table[index];
                table[index] = current;
             }
             ...
             if (i >= CAPACITY) {
                 rehash();
                 put(kicked.getBucKey(), kicked.getValue());
             }
        }
         return;
     }
