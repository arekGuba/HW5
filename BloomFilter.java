 ...
 ********************************************************************/
 ...
 class BloomFilter {
     ...
     public boolean contains(String s) {
         ...
         // of type BitSet (Java class ). See Oracle documentation for
         // this class on available methods. You can also see how method 'add'
         // in this class uses the object.

         for (int n = 0; n < noHashes; n++) {
            // hash function to find index value of hash of s
            long hc = hashCode(s, n);
            int bitNo = (int) (hc) & this.hashMask;
            // if a bit isn't found at the index values, s is not present,
            // loop ends early and returns false. otherwise, the loop will run
            // without terminating early, and then will return true as s is 
            // (most likely) present
            if (!data.get(bitNo)){
                return false;
            }
